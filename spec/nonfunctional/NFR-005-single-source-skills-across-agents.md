...
relationships:
  - target: "ix://agent-ix/ix-flow/US-010"
    type: "constrains"
---
...
ix-flow SHALL maintain a single skill tree under `skills/<name>/SKILL.md` as the
sole source of skill content, and each supported coding agent (Claude Code, OpenAI
Codex, opencode, GitHub Copilot) SHALL install from that one tree. Per-agent plugin
and marketplace manifests SHALL add only discovery and install metadata and SHALL
NOT contain a duplicated copy of any skill body.
...
- Operational context: installing ix-flow as a plugin from any supported agent.

## Rationale

A single skill tree is what lets ix-flow add agents without forking content: every
...
- **Upstream**: [US-010](../usecase/US-010-install-in-any-coding-agent.md) install
  ix-flow in the coding agent of choice.
- **Downstream**: the single-source rule governs how any future agent's manifest is
  added, alongside the packaging constraints in
  [FR-001](../functional/FR-001-cli-and-plugin-packaging.md).
