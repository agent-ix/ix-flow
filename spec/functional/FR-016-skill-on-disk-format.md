...
`contributes.workflows: <relative-dir>` declaration. If both declarations are
present, they SHALL use the same relative-directory value; conflicting
declarations SHALL fail with `skill_format_invalid`. The declared directory
SHALL hold one `workflows/<name>/def.yaml` per workflow and MAY include a
...
| FR-016-AC-5 | Only `invariants.js` ESM is supported, else `skill_script_unsupported`                                   | Analysis                      |
| FR-016-AC-6 | A single-workflow skill resolves without a name; ambiguous fails with `workflow_ambiguous`               | Analysis                      |
