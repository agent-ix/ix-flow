...
// human-readable stdout summary. Carries REAL token usage + a tool breakdown +
// ix-flow command classification.
...
export function buildResult(scenario, runs) {
  ...
  return {
    ...
    aggregate: {
      ...
      advances: summarize(samples.map((s) => s.advances)),
      gateAcks: summarize(samples.map((s) => s.gateAcks)),
      recordAnswers: summarize(samples.map((s) => s.recordAnswers)),
...
export function printSummaryTable(report) {
  ...
  console.log(
    "cols: adv=ix-flow advance · ack=ix-flow ack · ans=record-answers · edits=Write+Edit",
  );
}
...
export function rebuildFromTranscripts(prior, extractMetrics, meta) {
  const results = prior.results.map((res) => {
    ...
    const runs = res.runs.map((s) => ({
      ...
      assertion: { checks: s.checks, failures: s.failures },
      workdir: s.workdir,
